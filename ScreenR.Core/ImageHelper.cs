...
namespace ScreenR.Core
{
    ...
    {
        public static Result<SKBitmap> GetFrameDiff(SKBitmap currentFrame, SKBitmap? previousFrame)
        {
            try
            {
                ...
                {
                    return Result.Fail<SKBitmap>("Frames must be of equal dimensions and format.");
                }

                return Result.Ok(new SKBitmap());
            }
            catch
            {
                return Result.Fail<SKBitmap>("Failed to get image diff.");
            }
            finally
            {
