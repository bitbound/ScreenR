...
namespace ScreenR.Desktop.Windows.Capture
{
    public class ScreenGrabber : IScreenGrabber
    {
        ...
        public Result<SKBitmap> GetScreenGrab(string outputName)
        {
            try
            {
                ...
                if (display is null)
                {
                    return Result.Fail<SKBitmap>("Display name not found.");
                }
                ...
                {
                    result = GetBitBltGrab(display);
                    if (!result.IsSuccess || result.Value is null)
                    {
                        return Result.Fail<SKBitmap>(result.Exception ?? new("Unknown error."));
                    }
                }
        ...
        internal Result<Bitmap> GetBitBltGrab(DisplayInfo display)
        {
